<template>
    <div class="outbox"  :style="{height:(historyHeight-3)+'px'}">
       <div class="titlebox">
           <div class="line"></div>
           <div class="linefont">历史调度方案列表</div>
       </div>
...
export default {
  ...
  data() {
    return {
      titlelist:[
        {date:"日期",water:"总供水量(万m3/d)",lowpresure:"管网最低压力(MPa)",tataldh:"总单耗(kwh/m³·Mpa)"},
        {date:"时间",water:"供水量(m3/h)",lowpresure:"管网最低压力(MPa)",tataldh:"单耗(kwh/m³·Mpa)"}
      ],
      historylistHeight: 0,
     historyHeight:0,
      list: [
  ...
  mounted() {
    ...
    window.onresize = function() {
     ...
     Bus.$on("info6", e => {
      that.list = e.SchemeList; /* 　console.log(`传来的数据是：${e}`) */
    });
     Bus.$on("radionumdata", e => {
      that.radionindex= e; 　/* console.log(`传来的数据是：${e}`) */
  ...
  methods:{
    showdetaildata(listnowindex){
     /*  alert(listnowindex);
      ...
      var SchemeNo = this.fanganlibiaosuoyindata;
      var WaterworksNo="笔架山水厂";
      var SchemeDate=this.list[listnowindex].SchemeDate;
      var prelistd={SchemeNo,WaterworksNo,SchemeDate}
     this.prelistdata=prelistd;
     var prelistdatatotablist={SchemeNo,SchemeDate};
     ...
     ec5() {
      ...
      var _this = this;
      _this.$axios
        .post(urlClass.axiosUrYA + "GetWaterquantity", JSON.stringify(_this.prelistdata), {
          headers: { "Content-Type": "application/json;" }
        })
        .then(res => {
          var _this = this;
    ...
    ec12() {
      var _this = this;
      _this.$axios
        .post(urlClass.axiosUrYA + "GetOutPressure", JSON.stringify(_this.prelistdata), {
          headers: { "Content-Type": "application/json;" }
        })
        .then(res => {
          var _this = this;
     ...
     var _this = this;
      _this.$axios
        .post(urlClass.axiosUrYA + "GetWaterPoolLevel", JSON.stringify(_this.prelistdata), {
          headers: { "Content-Type": "application/json;" }
        })
        .then(res => {
          var _this = this;
      ...
      var _this = this;
      _this.$axios
        .post(urlClass.axiosUrYA + "GetWaterUnitConsumption", JSON.stringify(_this.prelistdata), {
          headers: { "Content-Type": "application/json;" }
        })
        .then(res => {
          var _this = this;
    ...
    ec15() {
      var _this = this;
      _this.$axios
        .post(urlClass.axiosUrYA + "GetWaterTotalEfficiency", JSON.stringify(_this.prelistdata), {
          headers: { "Content-Type": "application/json;" }
        })
        .then(res => {
          var _this = this;
     ...
     ec16() {
      var _this = this;
      _this.$axios
        .post(urlClass.axiosUrYA + "GetWaterPump", JSON.stringify(_this.prelistdata), {
          headers: { "Content-Type": "application/json;" }
        })
        .then(res => {
          var _this = this;
